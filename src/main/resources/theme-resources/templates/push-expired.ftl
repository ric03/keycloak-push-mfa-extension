        ...
        <div class="${properties.kcContentWrapperClass!}">
            <div class="kc-push-card">
                <div class="alert alert-error">
                    ${msg("push-mfa-expired-message")!"The push request has expired."}
                </div>
                <p class="kc-push-hint">${msg("push-mfa-expired-hint")!"Generate a new push request to continue."}</p>
                <form action="${url.loginAction}" method="post" class="kc-push-actions">
                    <button class="${properties.kcButtonClass!} ${properties.kcButtonPrimaryClass!}" type="submit" name="retry">${msg("doTryAgain")}</button>
                </form>
            </div>
        </div>
    </#if>
